...
contract SousChef {
    ...
    // Info of each user that stakes Choco tokens.
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        uint256 stakedSupply = choco.balanceOf(address(this));
    ...
    function updatePool() public {
        ...
        uint256 chocoSupply = choco.balanceOf(address(this));
        if (chocoSupply == 0) {
        ...
        poolInfo.accRewardPerShare = poolInfo.accRewardPerShare.add(tokenReward.mul(1e12).div(chocoSupply));
        poolInfo.lastRewardBlock = block.number;
    }

    // Deposit Choco tokens to SousChef for Reward allocation.
    function deposit(uint256 _amount) public {
        require (_amount > 0, 'amount 0');
        UserInfo storage user = userInfo[msg.sender];

        updatePool();
        choco.safeTransferFrom(address(msg.sender), address(this), _amount);
    ...
    // Withdraw Choco tokens from SousChef.
    function withdraw(uint256 _amount) public {
        ...
        updatePool();
        choco.safeTransfer(address(msg.sender), _amount);
    ...
    function emergencyWithdraw() public {
        UserInfo storage user = userInfo[msg.sender];
        choco.safeTransfer(address(msg.sender), user.amount);
