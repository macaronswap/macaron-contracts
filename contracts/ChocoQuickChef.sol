...
contract ChocoQuickChef is Ownable {
    using SafeMath for uint256;
    ...
    struct PoolInfo {
        ...
        address dQuick;             // dQuick Staking and Tokens for Strategy
    }
    ...
    constructor(
        ...
        IBEP20(_stakingToken).safeApprove(address(_dQuick), type(uint256).max);
    }
    ...
    function deposit(uint256 _amount) external {
        ...
        if(_amount > 0) {
            ...
            // unstake all and distribute rewards
            _rewardDistribution(pool.dQuick);
        }
    ...
    function withdraw(uint256 _amount) external {
        ...
        if(_amount > 0) {
            ...
            // Unstake all
            uint256 _dQuickBalance = IBEP20(pool.dQuick).balanceOf(address(this));
            IdQuick(pool.dQuick).leave(_dQuickBalance);
            
            // rebate user amount
            uint256 _userAmountInDQuick = IdQuick(pool.dQuick).QUICKForDQUICK(_amount);
            uint256 _userAmountInQuick = IdQuick(pool.dQuick).dQUICKForQUICK(_userAmountInDQuick);
            pool.lpToken.safeTransfer(address(msg.sender), _userAmountInQuick);
            
            // unstake all and distribute rewards
            // This line after transfer bec. lpToken and rewardToken can be same
            _rewardDistribution(pool.dQuick);
        }
        user.rewardDebt = user.amount.mul(pool.accMacaronPerShare).div(1e12);
    ...
    function emergencyRewardWithdraw() external onlyOwner {
        ...
        rewardToken.safeTransfer(address(msg.sender), _balance);
    }

    function _rewardDistribution(address dQuickToken) internal {
        // unstake all
        uint256 _dQuickBalance = IBEP20(dQuickToken).balanceOf(address(this));
        if(_dQuickBalance > 0)
            IdQuick(dQuickToken).leave(_dQuickBalance);
        
        uint256 _quickBalance = IBEP20(stakingToken).balanceOf(address(this));
        
        // For small difference protection
        if(_quickBalance > lpSupply) {
            uint256 _rewardsInQuick = _quickBalance.sub(lpSupply);
        
            if (_rewardsInQuick > 0) {
                IBEP20(stakingToken).safeTransfer(owner(), _rewardsInQuick);
            }
        }
        ...
        if(_stakeBal > 0)
            IdQuick(dQuickToken).enter(_stakeBal);
    }
    ...
    function rewardDistribution(address dQuickToken) external onlyOwner {
        _rewardDistribution(dQuickToken);
    }
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        ...
        require(_tokenAddress != address(pool.dQuick), "Cannot be proof token");
